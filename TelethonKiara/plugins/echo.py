...
from TelethonKiara.DB.echo_sql import (addecho, get_all_echos, is_echo,
                                      remove_echo)
...
async def echo(event):
    if event.reply_to_msg_id is not None:
        Kiara_X_Assistant, KIARA_USER, kiara_mention = await client_id(event)
        reply_msg = await event.get_reply_message()
        user_id = reply_msg.sender_id
        chat_id = event.chat_id
        if is_echo(user_id, chat_id):
            if gvarstat(f"ECHO_{Kiara_X_Assistant}") == "True":
                return await eod(event, "The user is already enabled with echo !")
        addecho(user_id, chat_id)
        addgvar(f"ECHO_{Kiara_X_Assistant}", "True")
