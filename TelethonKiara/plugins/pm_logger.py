from TelethonKiara.DB.pmlogger_sql import (add_nolog, del_nolog, get_all_nolog,
                                          is_nolog)
...
async def _(event):
    ...
    if Config.PM_LOGGER != 0:
        ...
        if event.reply_to_msg_id:
            reply_msg = await event.get_reply_message()
            await reply_msg.forward_to(Kiara_X_Assistant)
        elif event.pattern_match.group(1):
            user = f"#LOG | Chat ID: `{event.chat_id}`\n\n"
            textx = user + event.pattern_match.group(1)
            await event.client.send_message(Kiara_X_Assistant, textx)
        else:
            await parse_error(event, "Nothing given to save !")
            return
        await eod(event, "`Saved Successfully`")
