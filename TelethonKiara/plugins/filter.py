...
from TelethonKiara.DB.filter_sql import (add_filter, get_all_filters,
...
async def on_snip(event):
    ...
    if not gvarstat(f"FILTER_{Kiara_X_Assistant}_{str(_id)[1:]}"):
        return
...
async def on_snip_save(event):
    ...
    if msg:
        ...
        addgvar(f"FILTER_{Kiara_X_Assistant}_{str(_id)[1:]}", "TRUE")
        await eod(
            event,
...
async def on_all_snip_delete(event):
    ...
    try:
        delgvar(f"FILTER_{Kiara_X_Assistant}_{str(_id)[1:]}")
