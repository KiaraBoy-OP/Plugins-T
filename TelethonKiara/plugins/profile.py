...
async def _(event):
    Kiara_X_Assistant, KIARA_USER, _ = await client_id(event)
    user = await event.client.get_entity(Kiara_X_Assistant)
...
async def remove_profilepic(event):
    ...
    pfplist = await event.client(
        GetUserPhotosRequest(user_id=Kiara_X_Assistant, offset=0, max_id=0, limit=lim)
    )
    input_photos = []
...
async def stats(event):
    ...
    Kiara_X_Assistant, KIARA_USER, kiara_mention = await client_id(event, is_html=True)
