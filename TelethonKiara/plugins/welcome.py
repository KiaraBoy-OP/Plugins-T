...
from TelethonKiara.DB.welcome_sql import (add_welcome, get_current_welcome,
...
async def _(event):
    ...
    if not gvarstat(f"WELCOME_{Kiara_X_Assistant}_{str(event.chat_id)[1:]}"):
        return
...
async def save_welcome(event):
    ...
    if add_welcome(event.chat_id, 0, string, msg_id) is True:
        addgvar(f"WELCOME_{Kiara_X_Assistant}_{str(event.chat_id)[1:]}", "TRUE")
        return await eor(event, success.format("saved"))
    rm_welcome(event.chat_id)
    delgvar(f"WELCOME_{Kiara_X_Assistant}_{str(event.chat_id)[1:]}")
    if add_welcome(event.chat_id, 0, string, msg_id) is True:
        addgvar(f"WELCOME_{Kiara_X_Assistant}_{str(event.chat_id)[1:]}", "TRUE")
...
async def del_welcome(event):
    Kiara_X_Assistant, _, _ = await client_id(event)
    if gvarstat(f"WELCOME_{Kiara_X_Assistant}_{str(event.chat_id)[1:]}"):
