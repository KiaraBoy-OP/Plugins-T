...
def validate_session(session):
    if "==kiara" and "bot==" in session.lower():
        new_session = session[6:-5]
        return str(new_session)
    else:
        print(f"KIARABOT SESSION - Wrong session string!")
        sys.exit()


if Config.KIARABOT_SESSION:
    session = StringSession(validate_session(Config.KIARABOT_SESSION))
else:
    session = "kiarabot"

try:
    Kiara = TelegramClient(
        session=session,
